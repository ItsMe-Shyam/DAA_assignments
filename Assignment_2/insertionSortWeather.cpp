...
void insertionSort(vector<WeatherData>& data) {
    for (size_t i = 1; i < data.size(); ++i) {
        ...
        // Move elements of data[0..i-1] that are greater than key.city
...
void printWeatherData(const vector<WeatherData>& data) {
    cout << setw(5) << "S.No." << setw(15) << "City" << setw(15) << "Time Stamp" 
         << setw(15) << "Temp. (C)" << setw(15) << "Humidity (%)" << setw(15) << "Rainfall (mm)" << endl;
    for (size_t i = 0; i < data.size(); ++i) {
        cout << setw(5) << i + 1 << setw(15) << data[i].city << setw(15) << data[i].timeStamp 
             << setw(15) << data[i].temperature << setw(15) << data[i].humidity << setw(15) << data[i].rainfall << endl;
    }
}
